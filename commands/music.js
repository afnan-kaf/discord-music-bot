      ...
      if (!searchResults.length) {
        ...
        try {
          ...
          if (ytResults.items?.length) {
            ...
            if (!videoFound) return await searchMessage.edit('❌ No playable videos found! All videos failed extraction.');
          } else {
...
async function pause(message) {
  if (!message.member.voice.channel) return message.reply('❌ You need to be in a voice channel to pause music!');

  const serverQueue = musicPlayer.getQueue(message.guild.id);
  if (!serverQueue) return message.reply('❌ There is no music playing!');

  const success = musicPlayer.pauseSong(message.guild.id);
  if (success) message.reply('⏸️ Music paused!');
  else message.reply('❌ Nothing is playing!');
}

async function resume(message) {
  if (!message.member.voice.channel) return message.reply('❌ You need to be in a voice channel to resume music!');

  const serverQueue = musicPlayer.getQueue(message.guild.id);
  if (!serverQueue) return message.reply('❌ There is no music to resume!');

  const success = musicPlayer.resumeSong(message.guild.id);
  if (success) message.reply('▶️ Music resumed!');
  else message.reply('❌ Nothing to resume!');
}

async function stop(message) {
  if (!message.member.voice.channel) return message.reply('❌ You need to be in a voice channel to stop music!');
...
async function skip(message) {
  if (!message.member.voice.channel) return message.reply('❌ You need to be in a voice channel to skip music!');

  const serverQueue = musicPlayer.getQueue(message.guild.id);
  if (!serverQueue || !serverQueue.songs.length) return message.reply('❌ There is no music to skip!');

  const success = musicPlayer.skipSong(message.guild);
  if (success) message.reply('⏭️ Song skipped!');
  else message.reply('❌ Nothing to skip!');
}

async function showQueue(message) {
  const serverQueue = musicPlayer.getQueue(message.guild.id);
  if (!serverQueue?.songs.length) return message.reply('❌ The queue is empty!');
  ...
  if (serverQueue.songs.length > 10) queueList += `\n... and ${serverQueue.songs.length - 10} more songs`;
  embed.setDescription(queueList);
  await message.reply({ embeds: [embed] });
}

module.exports = { play, pause, resume, stop, skip, showQueue };
